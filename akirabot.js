...
const vcard = 'BEGIN:VCARD\n'  //Jangan diganti,Ntar error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=51935864086:+51 935 684 086\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
...
cr = '*Saiki bot By kytzia*'
vr = '*Saiki bot Wa*\n* < 𝙆𝙮𝙩𝙯𝙞𝙖 >*'

//owner number
const ownerNumber = ["51935684086@s.whatsapp.net"]  //Ganti Jadi Nomormu
...
┠⊷️ *ᴇʀɪ ɢᴇɪ?* :
┗━━━━━━━━━━━━━━━
 ʙɪᴇɴᴠᴇɴɪᴅᴏ ᴀ ᴇsᴛᴇ ʜᴇʀᴍᴏsᴏ ɢʀᴜᴘᴏ :3`
...
//ANTI VIRTEX 
		...
		if (isGroupAdmins) return reply('Grupo de admins')
		akira.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`VIRTEX TERDETEKSI!!! MAAF ${sender.split("@")[0]} ANDA AKAN DI KICK DARI GRUP DALAM WAKTU 5 s`)
		setTimeout( () => {
		...
		if (isGroupAdmins) return reply('ADMIN GRUP0')
		akira.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		reply(`VIRTEX TERDETEKSI!!! MAAF ${sender.split("@")[0]} ANDA AKAN DI KICK DARI GRUP DALAM WAKTU 5 s`)
		setTimeout( () => {
...
//CASE BADWORD
		...
		if (isBanned) return reply(ind.baned())
                ...
                addKoinUser('51935684086@s.whatsapp.net', fee)
...
fs.unlinkSync(rano)
  ...
  reply('y la imagen Tremendo Chupa gampi')
}
break
case 'triggered':
                    ini_url = args[0]
...
break
					...
					if (Number(args[0]) === 1) {
						if (isAntiLink) return reply('Ya está activado ._.')
						antilink.push(from)
		...
		if (isBanned) return reply(ind.baned())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('Que foto busco ano de alpaca con sida')
					pinte = body.slice(11)
		...
		case 'daftar':
                ...
                if(isNaN(umurUser)) return await reply('*Ya estás registrado._.*')       
                if (namaUser.length >= 30) return reply(`*why is your name so long it's a name or a train*`)
                if (umurUser > 20) return reply(`*senpai estás muy viejo >.<*`)
                if (umurUser < 10) return reply(`*estás pequeño y yo que t queria coger :<*`)
		    ...
		    addATM(sender)
                    ...
                    console.log(color('[REGISTER]'), color(time, 'yellow'), 'Name:', color(namaUser, 'Pink'), 'Age:', color(umurUser, 'cyan'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
                } else {
                    addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
                    await akira.sendMessage(from, pp_user, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
		    reply('Espero k t guste:3')
                    addATM(sender)
                    addLevelingId(sender)
...
` 
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
                   ...
                   if (!isNsfw) return reply(ind.nsfwoff())
					...
					if (Number(args[0]) === 1) {
						...
						if (!isAntiFirtex) return reply('Segurx?:^')
						var ini = anti.botLangsexOf(from)
					...
					if (Number(args[0]) === 1) {
						...
						akira.sendMessage(from,`ALLERT !!! ¡NO ENVÍE VIRUS !! PORQUE ESTE GRUPO ES ANTI VIRUS !! SI ENVIAR VIRUS SERÁN PATADAS !!`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntiFirtex3) return reply('Segurx?:^')
						var ini = anti.botLangsexOf(from)
					...
					if (Number(args[0]) === 1) {
						...
						if (!isAntiFirtex4) return reply('Segurx?:^')
						var ini = anti.botLangsexOf(from)
					...
					if (!isGroup) return reply(ind.groupo())					
					if (!isBotGroupAdmins) return reply('No soy akmin 😭')					
					if (args.length < 1) return reply('1 para activar .__.')
					if (Number(args[0]) === 1) {
						if (isAntiFirtex5) return reply('Ya activo..')
           	...
           	case 'setname':
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `bai gei nadie te quiere por eso t expluse :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						akira.groupRemove(from, mentioned)
					} else {
						mentions(`bai senpai t kieria pero me obligan a despedirte`, mentioned, true)
						akira.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
					if (!isGroup) return reply(ind.groupo())
					teks = `Lista de akmins *${groupMetadata.subject}*\n𝗧𝗼𝘁𝗮𝗹 : ${groupAdmins.length}\n\n`
					no = 0
		...
		if (isBanned) return reply(ind.baned())
                if (isGroup) return reply( 'Oye onichan para poder usar ese comando debemos estar en un grupo')
                anug = getRegisteredRandomId(_registered).replace('@s.whatsapp.net','')
                await reply('Looking for a partner...')
                await reply(`wa.me/${anug}`)
                await reply( `Partner found: 🙉\n*${prefix}next* — find a new partner`)
		...
		if (isBanned) return reply(ind.baned())
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ DESACTIVADO ❭ LOS EVENTOS HAN SIDP DESACTIVADOS*')
					} else {
						reply(ind.satukos())
					}
					break
				...
				case 'wait':
					...
					break
				default:
			if (budy.includes(`kytzia`)) {
                  reply(`es mi mujer calláte`)
                  }
		...
		akira.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
               ...
               reply(`A tus ordenes onichaan`)  
        const none = fs.readFileSync('./mp3/nemo.mp3');
		akira.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
                  }
          
                  
           if (budy.startsWith(`#rules`)) {
		...
		akira.sendMessage(from, none, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('[Saiki bot]','yellow'), ('Command Tidak Ditemukan!!','red'), color(sender.split('@')[0]))
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
		}
